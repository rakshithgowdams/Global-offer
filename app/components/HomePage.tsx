...
const nav = [
  ["Lead automation", "/real-estate-lead-automation"],
  ["AI calling", "/ai-calling-agent-real-estate"],
  ["CRM automation", "/real-estate-crm-automation"],
  ["Websites", "/real-estate-website-development"],
  ["Live demos", "/#demos"],
];
...
const services = [
  { n: "01", href: "/ai-calling-agent-real-estate", icon: "call" as IconName, eyebrow: "AI lead desk", title: "AI calling agents & conversational automation", body: "A 24/7 first-response layer for inbound and client-approved outreach. It answers common questions, qualifies intent, books conversations, logs outcomes and knows when to hand the lead to a person.", points: ["Inbound call handling and missed-call recovery", "Consent-based follow-up and reminders", "Multilingual flows for tested use cases", "Transcripts, summaries and quality review"] },
  { n: "02", href: "/real-estate-website-development", icon: "globe" as IconName, eyebrow: "Conversion experience", title: "Real-estate websites built to start conversations", body: "A premium, mobile-first website that moves visitors toward a viewing, valuation request or consultation instead of leaving them on a static brochure page.", points: ["Campaign and project landing pages", "Property enquiry and viewing funnels", "CRM, analytics and source attribution", "Performance, accessibility and search foundations"] },
  { n: "03", href: "/real-estate-crm-automation", icon: "database" as IconName, eyebrow: "Revenue operations", title: "CRM rescue, integration & custom CRM", body: "We first make the CRM you already pay for work properly. When your process is genuinely unique, we design a custom system around the way your brokerage operates.", points: ["Pipeline, fields, permissions and routing", "Lead-source integration and deduplication", "Tasks, alerts and management dashboards", "Custom CRM only where justified"] },
  { n: "04", href: "/real-estate-lead-automation", icon: "layers" as IconName, eyebrow: "Lead operations", title: "Real-estate lead automation that protects every enquiry", body: "Connect website, campaign, portal, call and referral leads to one controlled response, qualification, routing and follow-up process.", points: ["Website and Meta lead capture", "Qualification and opportunity scoring", "Agent routing and viewing booking", "Follow-up, reactivation and attribution"] },
  { n: "05", href: "/real-estate-marketing-automation", icon: "chart" as IconName, eyebrow: "Growth operations", title: "Marketing automation connected to the CRM", body: "For teams that need it, we connect approved campaigns and content to measurable lead capture, attribution and permission-aware follow-up.", points: ["Campaign planning and content workflow", "Website and Meta lead extraction", "Source tracking and CRM attribution", "Review, referral and reactivation journeys"] },
];
...
export default function HomePage() {
  ...
  const calendarUrl = "https://cal.com/rakshith-rfmgzr/real-estate-growth-automation";
    ...
    <main>
      ...
      <header className="site-header">
        <div className="shell header-inner">
          <Link className="brand" href="/" aria-label="MyDesignNexus PropertyFlow home"><span>MYDESIGNNEXUS</span><small>MDN PROPERTYFLOW</small></Link>
          <nav className="desktop-nav" aria-label="Primary navigation">{nav.map(([label, href]) => <a key={href} href={href}>{label}</a>)}</nav>
          <a className="button button-gold header-cta" href={calendarUrl} target="_blank" rel="noreferrer">Book a 60-min call <Icon name="calendar" size={17} /></a>
      ...
      <aside className={`mobile-menu ${menuOpen ? "is-open" : ""}`} aria-hidden={!menuOpen}>
        <nav>{nav.map(([label, href], i) => <a key={href} href={href} onClick={() => setMenuOpen(false)} style={{ "--i": i } as CSSProperties}><span>0{i + 1}</span>{label}</a>)}
          <a className="button button-gold mobile-menu-cta" href={calendarUrl} target="_blank" rel="noreferrer" onClick={() => setMenuOpen(false)}>Book a 60-min strategy call <Icon name="calendar" /></a>
        </nav>
      </aside>
        ...
        <div className="shell hero-layout" id="content">
          <div className="hero-copy">
            ...
            <div className="hero-actions">
              <a className="button button-gold button-large" href={calendarUrl} target="_blank" rel="noreferrer">Book a 60-min strategy call <Icon name="calendar" /></a>
              <button className="button button-outline button-large" onClick={() => setAuditOpen(true)}>Get your free workflow audit <Icon name="arrow" /></button>
            </div>
            <p className="booking-meta"><Icon name="calendar" size={16} /><strong>Google Meet</strong><span />Monday–Friday · 9:00 AM–5:00 PM Eastern · shown automatically in your timezone</p>
      ...
      <section className="section services-section" id="services">
        <div className="shell">
          ...
          <div className="service-list">{services.map((s)=><article className="service-card reveal" key={s.n}>
            ...
            <div className="service-copy"><small>{s.eyebrow}</small><h3><Link href={s.href}>{s.title}</Link></h3><p>{s.body}</p><Link className="service-detail-link" href={s.href}>Explore this real-estate solution <Icon name="arrow" size={17}/></Link></div>
      ...
      <section className="section faq-section" id="faq">
        <div className="shell faq-layout"><div className="faq-copy reveal"><p className="eyebrow">Questions before a decision</p><h2>Clear answers. No black-box promises.</h2><p>Every implementation begins with the real workflow, approved tools, responsible owners and the result you want to measure.</p><button className="text-link" onClick={()=>setAuditOpen(true)}>Ask a specific question <Icon name="arrow" size={18}/></button></div>
          <div className="faq-list">{homeFaqs.map((f,i)=><details className="faq-item reveal" key={f[0]}><summary><span>0{i+1}</span><strong>{f[0]}</strong><i>+</i></summary><p>{f[1]}</p></details>)}</div>
        </div>
      </section>

      <section className="final-cta" id="contact">
        <div className="grid-bg"/><div className="shell final-layout"><div className="final-copy reveal"><p className="eyebrow">Your next practical step</p><h2>Book a focused<br/>strategy session.</h2><p>Bring your lead sources, CRM and follow-up process. In one structured call, we will identify the highest-value automation opportunity and clarify what should be built first.</p><div><a className="button button-gold button-large" href={calendarUrl} target="_blank" rel="noreferrer">Choose a meeting time <Icon name="calendar"/></a><button className="text-link" onClick={()=>setAuditOpen(true)}>Start with the free audit <Icon name="arrow" size={18}/></button><a href={`https://wa.me/${phone}?text=${encodeURIComponent("Hello MyDesignNexus, I would like to discuss a real-estate automation system.")}`} target="_blank" rel="noreferrer"><Icon name="message" size={18}/>Prefer WhatsApp?</a></div></div>
          <article className="audit-preview strategy-preview reveal"><header><h3>Real Estate Growth & Automation</h3><span>60 minutes</span></header><ol>{["Review your current lead journey","Find response and follow-up leakage","Prioritise the strongest automation use case","Define practical next steps and ownership"].map((x,i)=><li key={x}><span>0{i+1}</span>{x}</li>)}</ol><p><Icon name="calendar" size={18}/><span><strong>Google Meet · U.S. client hours</strong><br/>Monday–Friday, 9:00 AM–5:00 PM Eastern. Cal.com converts every slot into the visitor’s local timezone.</span></p></article>
        </div>
      </section>

      <footer className="site-footer"><div className="shell footer-grid"><div className="footer-brand"><Link className="brand" href="/"><span>MYDESIGNNEXUS</span><small>MDN PROPERTYFLOW</small></Link><p>AI automation, calling agents, conversion websites, CRM integration and custom software for modern property teams.</p><small>Built in India. Serving ambitious real-estate teams worldwide.</small></div>
        <nav><p>Solutions</p>{nav.slice(0,4).map(x=><Link key={x[1]} href={x[1]}>{x[0]}</Link>)}<Link href="/real-estate-marketing-automation">Marketing automation</Link></nav><nav><p>Resources</p><Link href="/resources/real-estate-automation-guide">Real-estate automation guide</Link><Link href="/#demos">Interactive demos</Link><Link href="/about">About MyDesignNexus</Link><Link href="/#faq">Frequently asked questions</Link></nav><nav><p>Legal</p><Link href="/privacy">Privacy policy</Link><Link href="/terms">Terms & conditions</Link><Link href="/refunds">Refund policy</Link><Link href="/sitemap.xml">XML sitemap</Link></nav>
        <address><p>Start a conversation</p><a href={calendarUrl} target="_blank" rel="noreferrer">Book a 60-minute strategy call</a><a href="mailto:contact@mydesignnexus.in">contact@mydesignnexus.in</a><a href="tel:+918861241984">+91 886 1241 984</a><span>1st Floor, Gunde Gowdana Koppalu Circle,<br/>Ring Road, Hassan – 573201, India</span></address>
      ...
      <a className="mobile-sticky" href={calendarUrl} target="_blank" rel="noreferrer">Book a 60-min strategy call <Icon name="calendar" size={18}/></a>
